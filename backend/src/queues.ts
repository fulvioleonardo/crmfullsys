...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    logger.info(`Mensage agendado  para: ${schedule.contact.name}`);
...
async function handleVerifyCampaigns(job) {
  ...
  for (let campaign of campaigns) {
    try {
      ...
      logger.info(
        `Campaña enviada : Campanña=${campaign.id}, Delay Inicial=${delay}`
      );
      campaignQueue.add(
        "ProcessCampaign",
        {
...
async function handleDispatchCampaign(job) {
  try {
    ...
    logger.info(
      `Campaña enviada para: Campanña=${campaignId};Contato=${campaignShipping.contact.name}`
    );
  } catch (err: any) {
    Sentry.captureException(err);
...
async function handleVerifyQueue(job) {
  logger.info("Buscando Clientes perdidos ");
  try {
          ...
          if (moveQueue > 0) {
            ...
            if (!isNaN(idQueue) && Number.isInteger(idQueue) && !isNaN(timeQueue) && Number.isInteger(timeQueue)) {
              ...
              if (count > 0) {
                ...
                logger.info(`No se encontró ningún servicio perdido - Compañía: ${companyId}`);
              }
            } else {
...
async function handleRandomUser() {
  logger.info("Inicio de la aleatorización de la atención");
		...
		const updatedAtV = new Date(ticket.updatedAt);
          ...
          if (!userId) {
            ...
            const randomUserId = getRandomUserId(userIds);
              ...
              const msg = await SendWhatsAppMessage({ body: "*Chatbot Virtual*:\nEspere mientras localizamos a un asistente... ¡Será atendido en breve!", ticket: ticketToSend });
            ...
            if(tempoPassadoB > updatedAtV){
              ...
              if (availableUserIds.length > 0) {
                ...
                const randomUserId = getRandomUserId(availableUserIds);
                  ...
                  const ticketToSend = await ShowTicketService(ticket.id, ticket.companyId);
                  const msg = await SendWhatsAppMessage({ body: "*Chatbot Virtual*:\n...  será atendido em breve!", ticket: ticketToSend });
