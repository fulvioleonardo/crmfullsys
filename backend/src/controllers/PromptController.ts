...
export const remove = async (
  ...
  try {
    ...
    if (count > 0) return res.status(200).json({ message: "¡No es posible eliminarlo! Verifique si este mensaje está sendo usado nas conexiones Whatsapp!" });
    ...
    return res.status(500).json({ message: "¡No es posible eliminarlo! Verifique que este mensaje se esté usando." });
  }
};
