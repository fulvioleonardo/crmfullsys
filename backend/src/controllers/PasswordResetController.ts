...
export const updatePassword = async (req: Request, res: Response) => {
  ...
  console.log('Datos recibidos para actualización de contraseña:', userId, newPassword);
...
export const getVerificationData = (req: Request, res: Response) => {
  ...
  if (verificationCodes[email]) {
    ...
    res.status(404).json({ error: 'No se encontró ningún código de verificación para el correo electrónico proporcionado.' });
  }
};
...
export const sendCodeVerifycation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const message = `\u200e Tu código de verificación é: ${verificationCode}`;
