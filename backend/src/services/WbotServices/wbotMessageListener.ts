...
export const verifyMediaMessage = async (
  ...
  try {
    ...
    try {
      ...
      await writeFileAsync(join(folder, media.filename), media.data.toString('base64'), "base64") // Correção adicionada por Altemir 16-08-2023
        ...
        .then(() => {
          ...
          if (inputFile.endsWith(".mpeg")) {
            ...
            console.error("Formato de archivo no compatible:", inputFile);
            return;
          }
      ...
      Sentry.setExtra('Error de medios', { companyId: companyId, ticket, contact, media, quotedMsg });
    ...
    console.log(error);
    logger.warn("Error al descargar medios");
  }
};
...
${JSON.stringify(msg?.message)}`);
    ...
    Sentry.setExtra("El error isValidMsg", { msg });
    Sentry.captureException(error);
...
const verifyQueue = async (
  ...
  if (queues.length === 1 && !chatbot) {
    ...
    if (enableQueuePosition) {
      ...
      const msgFila = `*Assistente Virtual:*\n{{ms}} *{{name}}*, su posición en la cola de atención es: *${qtd}*`;
  ...
  const botText = async () => {
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      if (enableQueuePosition && !choosenQueue.chatbots.length) {
        // Lógica para enviar posição da fila de atendimento
        const qtd = count.count === 0 ? 1 : count.count
        const msgFila = `*Asistente virtual:*\n{{ms}} *{{name}}*, Su posición en la cola de servicio es: *${qtd}*`;
        const bodyFila = formatBody(`${msgFila}`, ticket);
        const debouncedSentMessagePosicao = debounce(
          async () => {
            await wbot.sendMessage(
      ...
      if (ticket.whatsapp.greetingMediaAttachment !== null) {
        ...
        if (ticket.amountUsedBotQueues > 0) {
          ...
          await wbot.sendMessage(`${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, {
            text: 'Opción no válida, elija una opción válida a continuación:',
          });
...
const handleMessage = async (
  ...
  if (isImported) {
    addLogs({ fileName: `processImportMessagesWppId${wbot.id}.txt`, text: `Importando mensaje: ${JSON.stringify(msg, null, 2)}>>>>>>>>>>>>>>>>>>>` })
    ...
    if (existMessage) {
      ...
      console.log("Este mensaje ya existe")
      return
    } else {
  ...
  try {
    ...
    if (msgType === "editedMessage") {
      ...
      try {
        ...
        Sentry.captureException(err);
        logger.error(`Error al manejar el mensaje de confirmación. Error: ${err}`);
      }
      return
    }
      ...
      if (!msg.key.fromMe && settings.scheduleType && (!ticket.isGroup || whatsapp.groupAsTicket === "enabled")) {
        ...
        if (
          ...
          if (whatsapp.outOfHoursMessage !== "") {
            ...
            //Actualiza el contador de veces que se envió el bot y se envió fuera de tiempo
            await ticket.update({
              isOutOfHour: true,
              amountUsedBotQueues: ticket.amountUsedBotQueues + 1
            });
    ...
    if (
      ...
      const sentMessage = await wbot.sendMessage(
        `${contact.number}@c.us`,
        {
          text: `\u200e*Asistente virtual*:\nDesafortunadamente no podemos escuchar ni enviar audio a través de este canal de servicio. Envíe un mensaje. *texto*.`
        },
        {
          quoted: {
            key: msg.key,
    ...
    Sentry.captureException(err);
    console.log(err);
    logger.error(`Error al manejar el mensaje de WhatsApp: Error: ${err}`);
  }
};
    ...
    messages.forEach(async (message: proto.IWebMessageInfo) => {
      ...
      if (!messageExists) {
        ...
        if (fromMe) {
          ...
          if (/\u200c/.test(body))
            body = body.replace(/\u200c/, '')
          logger.debug('Validación de mensajes de campaña enviados por terceros: ' + body)
        }
...
const handleOpenAi = async (msg: proto.IWebMessageInfo, wbot: Session, ticket: Ticket, contact: Contact, mediaSent: Message | undefined, ticketTraking: TicketTraking): Promise<void> => {
  ...
  const promptSystem = `En tus respuestas utiliza el nombre. ${sanitizeName(contact.name || "Amigo(a)")} para identificar o cliente.\nSTu respuesta debe usar como máximo ${prompt.maxTokens}
